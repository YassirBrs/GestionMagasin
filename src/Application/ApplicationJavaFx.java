...
import ClientField.ClientDiplay;
import ProduitField.*;
...
import CategorieField.CategorieDisplay;
...
//        });
		link4.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent t) {
            	ProduitAdd frmJPrd = new ProduitAdd();
                Stage w = new Stage();
                Stage stage; 
                try {
                	if(bProduct == true){
                		if(ProduitAdd.message != null){
			                stage = (Stage) ProduitAdd.message.getScene().getWindow();
			                if(!stage.isShowing()){
			                	frmJPrd.start(w);}
			        }}
                } catch (Exception e) { System.out.println("Erreur !"); }
		...
		link5.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent t) {
            	if(produitSelected != null){
            		ProduitEdit frmMPrd = new ProduitEdit();
	                Stage w = new Stage();
	                Stage stage; 
	                try {
	                	if(bUpdate == true){
	                		if(ProduitEdit.message != null){
				                stage = (Stage) ProduitEdit.message.getScene().getWindow();
				                if(!stage.isShowing()){
				                	frmMPrd.start(w);}
				        }}
	                } catch (Exception e) { System.out.println("Erreur !"); }
		...
		link8.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent t) {
            	CategorieDisplay frmCat = new CategorieDisplay();
                Stage w = new Stage();
                Stage stage; 
                try {
                	if(bCat == true){
                		if(CategorieDisplay.message != null){
			                stage = (Stage) CategorieDisplay.message.getScene().getWindow();
			                if(!stage.isShowing()){
			                	frmCat.start(w);}
			        }}
                } catch (Exception e) { System.out.println("Erreur !"); }
...
//		pane.getChildren().addAll(pt,link1,link4,link5,link6,link7,link8);
		pane.getChildren().addAll(pt,link4,link5,link6,link7,link8);
		return pane;
	}

	private void createContentCenter(){
		try {
			welcomeImg = Tools.createImageView("photosStock/stoore.png");
			welcomeImg.setFitHeight(350);
			welcomeImg.setFitWidth(500);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		};
